...
int main( void )
{
	...
	{
		// make table widgets
		SGUI_Label_new(&lbl_userid[i], &mnu_users, font, TEST_THEME->label);
		SGUI_Entry_new(&txt_name[i], &mnu_users, font, TEST_THEME->entry);
		SGUI_Button_new(&btn_delete[i], &mnu_users, font, TEST_THEME->button);
